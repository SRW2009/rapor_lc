...
class NHBRepositoryImpl extends NHBRepository {
    ...
    final response = await http.post(
      ...
      body: jsonEncode({
        ...
        'query': '''
          ...
          `nilai_harian`, `nilai_bulanan`, `nilai_projek`, `nilai_akhir`, `akumulasi`, 
          `predikat`) 
    ...
    final response = await http.post(
      ...
      body: jsonEncode({
        ...
        'query': '''
          ...
          `akumulasi`, `predikat` 
          FROM tb_nilai_hasil_belajar as nhb WHERE id=$id
        ''',
      }),
    );
  ...
  @override
  Future<List<NHB>> getNHBList(String santriNis) async {
    final token = await SharedPrefsRepository().getToken;
    ...
    final response = await http.post(
      ...
      body: jsonEncode({
        ...
        'query': '''
          ...
          `nilai_harian`, `nilai_bulanan`, `nilai_projek`, `nilai_akhir`, 
          `akumulasi`, `predikat` 
          FROM tb_nilai_hasil_belajar as nhb WHERE santri_nis='$santriNis'
        ''',
      }),
    );
    if (response.statusCode == StatusCode.getSuccess) {
  ...
  @override
  Future<RequestStatus> updateNHB(NHB nhb) async {
    final token = await SharedPrefsRepository().getToken;
    ...
    final response = await http.post(
      ...
      body: jsonEncode({
        'query_type': DataConstant.queryType_action,
        'query': '''
          UPDATE `tb_nilai_hasil_belajar` 
          SET `santri_nis`='${nhb.santri.nis}',`semester`=${nhb.semester},
          `tahun_ajaran`='${nhb.tahun_ajaran}',`mata_pelajaran_id`=${nhb.pelajaran.id},
          `nilai_harian`=${nhb.nilai_harian},`nilai_bulanan`=${nhb.nilai_bulanan},`nilai_projek`=${nhb.nilai_projek},
          `nilai_akhir`=${nhb.nilai_akhir},`akumulasi`=${nhb.akumulasi},`predikat`='${nhb.predikat}' 
          WHERE `id`=${nhb.id}
        ''',
      }),
    );
