...
class ClientNHBCreateDialog extends StatefulWidget {
  final Function(NHB) onSave;
  final ManageNHBController controller;
  final int lastIndex;

  const ClientNHBCreateDialog({Key? key, required this.onSave, required this.controller, required this.lastIndex,
  }) : super(key: key);

  @override
  State<ClientNHBCreateDialog> createState() => _ClientNHBCreateDialogState();
}

class _ClientNHBCreateDialogState extends State<ClientNHBCreateDialog> {
      ...
      contents: [
        ...
        BaseDialogActions(
          ...
          onSave: () => widget.onSave(
            NHB(widget.lastIndex, _mapelCon!, int.tryParse(_nilaiHarianCon.text)!,
