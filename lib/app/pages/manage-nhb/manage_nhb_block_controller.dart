...
import 'package:rapor_lc/app/dialogs/common/nhb_block_create_dialog.dart';
import 'package:rapor_lc/app/dialogs/common/nhb_block_update_dialog.dart';
...
class ManageNHBBlockController extends DataTableController<NHBBlock> {
  final Nilai _nilai;
  bool tableHasChanged = false;

  final ManageNHBPresenter _presenter;
  ManageNHBBlockController(nilaiRepo, mapelRepo, this._nilai)
      : _presenter = ManageNHBPresenter(nilaiRepo, mapelRepo),
        super.initial(
          (_nilai.nhbBlock..sort((a, b) => a.no.compareTo(b.no))).toList()
      );
  ...
  void onSave() {
    _nilai.nhbBlock = normalList.toList();
  ...
  void _doCreateNHB(NHBBlock item) {
    normalList.add(item);
    tableHasChanged = true;
    refresh();
  }
  void _doUpdateNHB(NHBBlock item) {
  ...
  @override
  Widget? createDialog() => NHBBlockCreateDialog(
    lastIndex: (normalList.isEmpty) ? 0 : normalList.last.no+1,
    onFindMapel: dialogOnFindMapel,
    onSave: (NHBBlock item) => _doCreateNHB(item),
  );

  @override
  Widget? updateDialog(NHBBlock? e) => NHBBlockUpdateDialog(
    nhb: e!,
    onFindMapel: dialogOnFindMapel,
    onSave: (NHBBlock item) => _doUpdateNHB(item),
  );
  ...
  @override
  String getSelectedKey(NHBBlock e) => e.no.toString();

  @override
  bool searchWhereClause(NHBBlock e) => e.no.toString().contains(currentQuery)
      ...
      || e.description.toLowerCase().contains(currentQuery);
}