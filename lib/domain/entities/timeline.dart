...
class Timeline implements Comparable {
  ...
  factory Timeline.fromString(String initial) {
    ...
    if (strings.length == 4) {
      ...
      if (strings[1].startsWith('s'))
        semester = int.tryParse(strings[1].substring(1));
      if (strings[2].startsWith('k'))
        kelas = int.tryParse(strings[2].substring(1));
      if (strings[3].startsWith('l'))
        level = int.tryParse(strings[3].substring(1));
    ...
    if (strings2.length == 4) {
      ...
      semester = int.tryParse(
          strings2[1].replaceAll('Semester', '').replaceAll(' ', ''));
      kelas =
          int.tryParse(strings2[2].replaceAll('Kelas', '').replaceAll(' ', ''));
      level =
          int.tryParse(strings2[3].replaceAll('Level', '').replaceAll(' ', ''));
      if (bulan != null && semester != null && kelas != null && level != null) {
        return Timeline(bulan, semester, kelas, level);
      }
    }
  ...
  @override
  int get hashCode =>
      bulan.hashCode ^ semester.hashCode ^ kelas.hashCode ^ level.hashCode;
